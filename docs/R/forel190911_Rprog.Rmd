...
* Månedlig indtægt efter skat fra studiejob for studerende på StatData1 i 2019: beregninging af konfidensinterval, hvor der trækkes på den centrale grænseværdisætning
...
Vi når næppe igennem alle detaljer ved dagens forelæsning, men det bør også være muligt at få rigtig meget ud af, at læse de resterende ting igennem selv derhjemme. Til analysen af det tredje eksempel (gæt antal punkter) benyttes R koden fra Helle Sørensen (tidl. underviser) stort set uændret.
...
## Månedlig indtægt fra studiejob på

Ved forelæsningen d. 9/9-2019 på StatData1 i 2019 blev der indsamlet et lille datasæt, hvor en del af de studerende blev besvarende følgende spørgsmål 

> Angiv din gennemsnitlige månedlige indtægt (i danske kroner, efter skat) fra dit studiejob.

Datasættet `studiejob.xlsx` består af 51 målinger af den månedlige indtægt (`indtaegt`). Datasættet indholder to meget store værdier (30000, 42000) der må formodes at være årlige indtægter. Jeg har tilladt mig at dividere disse tal med 12 før indlæsning af datasættet.

```{r message = FALSE}
library(tidyverse)
library(readxl)
studiejob <- read_excel("../data/studiejob.xlsx")
```

Lad os benytte datasættet til at komme med et bud på den gennemsnitlige indtægt per måned fra studiejob for populationen af alle studerende.

Det er let at beregne gennemsnittet i vores sample og benytte dette som *punktestimat* ...

```{r}
mean(studiejob$indtaegt)
```
...
Vi laver et histogram og et QQ-plot som vi lærte ved forelæsningen d. 9/9-2019.

```{r}
par(mfrow = c(1, 2))
hist(studiejob$indtaegt, prob = TRUE, xlab = "Maanedlig indtaegt (i DKK)", main = "Studerende paa StatData1 i 2019", col = "lightgrey")
qqnorm(studiejob$indtaegt, main="Studerende paa StatData1 i 2019")
abline(mean(studiejob$indtaegt), sd(studiejob$indtaegt), lwd = 3, col = "red")
```

**Konklusion:** Målingerne af den månedlige indtægt fra studiejob kan absolut ikke beskrives med en normalfordeling.

Formlerne i lærebogens Kap. 5.3 der angiver, hvordan vi skal lave et konfidensinterval for gennemsnittet er udledt under den antagelse, at målingerne i vores stikprøve kan betragtes som *uafhængige observationer fra en normalfordeling*. Betyder det, at vi ikke kan formlerne til beregning af konfidensintervaller for den gennemsnitlige indtægt fra studiejob?
...
Vores datasæt består og 51 målinger, hvilket må betragtes som en rimelig stor stikprøve, så der er god grund til at tro, at vi blot kan beskrive gennemsnittet ved en normalfordeling.

Til illustration af pointen kan vi lave et legetøjseksperiment. Lad os for et øjeblik antage at de 51 udgør den fulde population, som vi interesserer os for. Histogrammet her ...
...
mean_toy <- rep(NA, B)
for(b in 1:B){
  mean_toy[b] <- mean(sample(studiejob$indtaegt, size = 10, replace = T))
}

par(mfrow = c(1, 2))
hist(mean_toy, prob = TRUE, xlab = "Gennemsnitlig indtaegt i stikproeven (i DKK)", main = "Legetoejspopulation", col = "lightgrey")
...
**Konklusion:** Vi ser at gennemsnit for en stikprøve af størrelse 10 i langt bedre grad kan beskrives ved en normalfordeling, en hvis vi betragter de enkelte (51) målinger. Det er bestemt ikke lige så pænt, som da vi betragtede højdemåingerne for kvinder på StatData1  i 2017. Vi bør dog huske på, at vi i praksis interesserer os for gennemsnittet af indtægten fra studiejob i hele populationen af studerende, og at vi i praksis beregner et gennemsnit af 51 målinger (modsat kun 10 i stikprøverne i legetøjseksperimentet). Vi kan derfor regne med, at vores gennemsnit af 51 målinger vil ligne en normalfordeling endnu mere end de to figurer fra legetøjseksperimentet.


Således oplyst vover vi pelsen og bruger alligvel formlen fra lærebogens kapitel 5.3 til at beregne et 95 % konfidensinterval for den gennemsnitlige indtægt fra studiejob for populationen af alle studerende.

### Konfidensinterval for gennemsnitlig månedlig indtægt fra studiejob i populationen af studerende

Punktestimatet for den gennemsnitlige månedlig indtægt er
...
```{r}
### Nedre og øvre grænse, manuelt
mean(studiejob$indtaegt) - 2.008559 * sd(studiejob$indtaegt) /sqrt(51)
mean(studiejob$indtaegt) + 2.008559 * sd(studiejob$indtaegt) /sqrt(51)
```

Hvis vi ellers kan regne med at målingerne i vores stikprøve er repræsentativ for hele studenterpopulationen, så vil estimatet 3489 kr samt 95 % - konfidensintervallet 2779 - 4158 kr være en bud på denne gennemsnitlige månedlige indtægt (efter skat) fra studiejob.
