...
Eksemplet blev første gang diskuteret ved forelæsningen d. 23/10-2019 på Statistisk Dataanalyse 1
...
For en 2x2 tabel laver `chisq.test` som default en kontinuitetskorrektion. Hvis vi ikke vil have det, skal vi bruge optionen `correct=FALSE. Ved forelæsningerne snakker vi kun om versionen uden kontinuitetskorrektion.
...
p-værdien bliver hhv. 0.0039 og 0.0074, så hypotesen forkastes. Der er forskel på sandsynlighederne;
risikoen er størst for kastrerede mus.
...
# Alkohol og forventninger til Statistisk Dataanalyse 1

## Beskrivelse af data

Ved første forelæsningen i Statistisk Dataanalyse 1 d. 2/9-2019 blev der indsamlet diverse information omkring nogle af de studerende, som var tilstede ved forelæsningen. 

Nedenfor fokuserer vi på svarene på følgende spørgsmål

* Har du glædet dig til StatData1? (ja/nej)
* Hvor tit drikker du alkohol? \\
(A: aldrig, B : 0-1 gang/uge, C: 2- gange/uge)

Det fulde datasæt kan findes under Data på kursushjemmesiden. Bemærk, at der under R på kursushjemmesiden findes et link, der beskriver oprensningen af de rå data, som skal foretages inden man kan analysere data. Kun det rensede datasæt (fra filen `StatData1stud2019.txt`) benyttes her.

Når vi inddeler svarene fra 110 studerende efter de to inddelingskriterier (forventning til SD1 og alkoholforbrug) fås følgende tabel.

```{r}
sd1data <- read.table(file = "../data/StatData1stud2019.txt", header = T)
sd1tabel <- table(sd1data$sd1, sd1data$alkohol)
sd1tabel
```

Vi ønsker at teste hypotesen om, at svarende på de to spørgsmål er uafhængige af hinanden. Se mere på dagens forelæsningsslides for detaljer omkring, hvordan man kan formulere denne hypotese matematisk.

## Test for uafhængighed

Bemærk at data ovenfor er indlæst som en matrix (kaldet `sd1tabel`), denne gang som en 2x3 matrix. Du kan også selvom indtaste datasæt *søjlevis* som beskrevet på slides til dagens forelæsning. Check altid at tabellen/matricen er blevet som den skal være.
...
Vi konkluderer at  hypotesen om at de to inddelingskriterier er uafhængige ikke bør forkastes ($\chi^2 = 4.0725$, p = 0.13).
...
Når vi bruger `chisq.test`, får vi en p-værdi på 0.13, men vi får også en advarsel (`Warning`) om at $\chi^2$- approksimationen er tvivlsom. Det er fordi nogle af de forventede værdier er mindre end 5 (dog kun en enkelt). Vi er derfor ikke ganske sikre på validiteten af p-værdien.
...
Så får vi en p-værdi på omkring 0.12. Bemærk at vi ikke får præcis det samme hver gang, medmindre vi som i R koden vælger et fast `seed` (her vha. `set.seed(2019)`)

```{r}
set.seed(2019)
chisq.test(sd1tabel, simulate.p.value = TRUE, B=10000)
```

p-værdierne fra de to forskellige metoder er ret ens (0.13 vs cirka 0.12), så vi får samme konklusion.
