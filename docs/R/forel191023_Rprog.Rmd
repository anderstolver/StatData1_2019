...
Eksemplet blev første gang diskuteret ved forelæsningen d. 21/10-2019 på Statistisk Dataanalyse 1
...
For en 2x2 tabel laver `chisq.test` som default en kontinuitetskorrektion. Hvis vi ikke vil have det, skal vi bruge optionen `correct=FALSE`. Ved forelæsningerne snakker vi kun om versionen uden kontinuitetskorrektion (-se dog slide 44-45 fra 23/10-2019).
...
Det fulde datasæt kan findes under menupunktet **Data** på kursushjemmesiden. Bemærk, at der under menupunktet **R** på kursushjemmesiden findes et link, der beskriver oprensningen af de rå data, som skal foretages inden man kan analysere data. Kun det rensede datasæt (fra filen `StatData1stud2019.txt`) benyttes her.
...
**Simulering af p-værdi ved tynde tabeller:** Når vi bruger `chisq.test`, får vi en p-værdi på 0.13, men vi får også en advarsel (`Warning`) om at $\chi^2$- approksimationen er tvivlsom. Det er fordi nogle af de forventede værdier er mindre end 5 (dog kun en enkelt på 3.4). Vi er derfor ikke ganske sikre på validiteten af p-værdien.
