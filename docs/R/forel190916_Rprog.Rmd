...
Her vises hvordan man kan estimere parametrene i en ensidet variansanalysemodel (ANOVA). Vi regner på datasættet `antibio` der bl.a. blev diskuteret på slides og R program til forelæsningen d. 4/9-2019 (-se også s. 53 og 114 i lærebogen).
...
* I *Coefficients* tabellen hører alle tal i en linje altid til samme parameter i modellen. Fx: I model2 er første parameter middelværdien i kontrolgruppen, og både estimat og SE hører derfor til denne parameter. Parametren hørende til linje to er derimod *forskellen* mellem `Alfacyp` og `kontrol`, og både estimat og SE hører til denne parameter. Output fra *confint* hører til de samme parametre, og i samme rækkefølge.

Eksempler på fortolkning af resultater

* For alle typer antibiotika estimeres den forventede værdi af tilbageværende organisk materiale *højere* end for kontrol, og for alle typer pånær `Enroflox` indeholder de tilhørende konfidensintervaller indeholder ikke 0. Fx estimeres middelværdien 0.230 (SE 0.070, 95% KI (0.086, 0.274)) højere for `Fenbenda` end for kontrol.
...
Det (ukendte) parameter a og b der beskriver regressionslinjen estimeres ved mindste kvadraters metode fx. når man benytter `lm()`-funktionen i R som nedenfor.
...
* Gennemsnitsvægten blandt katte på 2.5 kg estimeres til 9.72 g med 95% konfidensinterval (9.46, 9.99). Bemærk at dette interval udtaler sig om gennemsnittet for katte med kropsvægt på 2.5 kg - ikke om værdien for en enkelt kat.