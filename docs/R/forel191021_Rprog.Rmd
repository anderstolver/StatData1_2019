...
Hvis n=8, så vi i steder ser på bin(8,0.6) får vi i stedet følgende:
...
Der var 62 studerende, som deltog i quizzen og havde mulighed for at besvare spørgsmålet. Der var 6 mulige svarkategorier og 16 af de 62 studerende svarede korrekt.

Vi lader $Y$ betegne antallet af studerene, som svarer korrekt og benytter den statistiske model, hvor $Y \sim bin(62, p)$ hvor parameteren p der beskriver successandsynligheden (i hver *trial*) opfattes som ukendt.

## Estimat og simpelt KI 

Vi laver beregningerne manuelt (baseret på s. 316-317 i+ Ex. 11.7 i lærebogen) og får $\hat p=0.258$ og 95% konfidensinterval (0.149, 0.367).
...
Vi beregner først det forbedrede KI manuelt (lærebogens kapitel 11.3.1) og får (0.165, 0.380).

```{r}
p1tilde <- (16+2)/(62+4)
p1tilde - 1.96 * sqrt(p1tilde * (1-p1tilde) / 66)
p1tilde + 1.96 * sqrt(p1tilde * (1-p1tilde) / 66)
```

Derefter bruger vi `prop.test` og får (0.166, 0.379).
...
* Vi benytter observationen 16 selv som teststørrelse og summerer alle de punktsandsynligheder der er $\leq P(Y=16)$ 
...
```{r}
prop.test(16, 62, correct=FALSE, p=1/6)
```
...
Eksemplet er diskuteres på slides til forelæsningen d. 21/10-2019 på Statistisk Dataanalyse 1
